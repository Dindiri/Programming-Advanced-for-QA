using NUnit.Framework;

using System;
using System.Collections.Generic;

namespace TestApp.UnitTests;

public class PersonTests
{
    Person _person;

        [SetUp]
        public void SetUp() 
        {
        _person = new Person();
        }


    // TODO: finish test
    [Test]
    public void Test_AddPeople_ReturnsListWithUniquePeople()
    {
        // Arrange
        string[] peopleData = { "Alice A001 25", "Bob B002 30", "Ali A001 35" };
        List<Person> expectedPeopleList = new List<Person>()
        {
           { new Person() { Name = "Ali", Id = "A001", Age = 35 } },
           { new Person() { Name = "Bob", Id = "B002", Age = 30 } }
        };

        // Act
        List<Person> resultPeopleList = this._person.AddPeople(peopleData);

        // Assert
        Assert.That(resultPeopleList, Has.Count.EqualTo(2));
        for (int i = 0; i < resultPeopleList.Count; i++)
        {
            Assert.That(resultPeopleList[i].Name, Is.EqualTo(expectedPeopleList[i].Name));
            Assert.That(resultPeopleList[i].Id, Is.EqualTo(expectedPeopleList[i].Id));
            Assert.That(resultPeopleList[i].Age, Is.EqualTo(expectedPeopleList[i].Age));
        }
    }

    [Test]
    public void Test_GetByAgeAscending_SortsPeopleByAge()
    {
        // Arrange
        List<Person> expectedPeopleList = new List<Person>()
        {
    
           { new Person() { Name = "Alice", Id = "A002", Age = 45 } },
           { new Person() { Name = "Bob", Id = "B002", Age = 30 } },
           { new Person() { Name = "Ali", Id = "A001", Age = 35 } },
           { new Person() { Name = "Robert", Id = "C004", Age = 25 } }
        };

        string expectedPeople = $"Robert with ID: C004 is 25 years old.{Environment.NewLine}" +
                                $"Bob with ID: B002 is 30 years old.{Environment.NewLine}" +
                                $"Ali with ID: A001 is 35 years old.{Environment.NewLine}" +
                                $"Alice with ID: A002 is 45 years old.";

        // Act
        string result = _person.GetByAgeAscending(expectedPeopleList);

        // Assert
        Assert.That(result, Is.EqualTo(expectedPeople));
    }
}
